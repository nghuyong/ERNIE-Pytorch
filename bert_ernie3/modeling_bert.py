...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config,task_type_vocab_size=3,
                 task_id=0,
                 use_task_id=False):
        super().__init__()
    ...
    def forward(
        ...
        if self.use_task_id:
            if task_type_ids is None:
                task_type_ids = torch.ones_like(input_ids) * self.task_id
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.segment_type == 'relative':
            ...
            relative_segment_scores = torch.where(token_type_distance > 0, segment_embedding[..., 1:], segment_embedding[..., 0:1])
